...
# Spinlock class for busy waiting
class Spinlock:
    def __init__(self):
        self.flag = False
...
def worker1(spinlock, shared_data, latency_results, start_event, end_event):
...
def worker2(spinlock, shared_data, end_event):
...
def measure_spinlock_latency(num_iterations):
    spinlock = Spinlock()
    ...
    thread1 = Thread(target=worker1, args=(spinlock, shared_data, latency_results, start_event, end_event))
    thread2 = Thread(target=worker2, args=(spinlock, shared_data, end_event))
...
def save_results_to_csv(results, num_iterations):
    ...
    filename = f"../results/spinlock_latency_{num_iterations}_{timestamp}.csv"
...
if __name__ == "__main__":
    ...
    # Measure latencies
    results = measure_spinlock_latency(args.num_iterations)
