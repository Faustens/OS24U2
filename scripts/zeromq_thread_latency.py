...
def measure_latency(num_iterations):
    ...
    start_event = th.Event()
    latencies = []

    thread1 = th.Thread(target=publisher, args=(num_iterations, start_event))
    thread2 = th.Thread(target=subscriber, args=(num_iterations, latencies, start_event))
...
def save_results_to_csv(results, num_iterations):
    ...
    filename = f"../results/zeromq_thread_latency_{num_iterations}_{timestamp}.csv"
